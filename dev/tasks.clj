(ns tasks
  ...
  This is basically the full tasks.clj copied from the biff source, with a few adjustments to allow
...
(defn- ssh-run [{:keys [biff.tasks/server biff.tasks/ssh-port]} & args]
  (apply shell "ssh"
         (concat (when ssh-port ["-p" (str ssh-port)])
                 [(str "app@" server)]
                 args)))
...
(defn- push-files-rsync [{:biff.tasks/keys [server deploy-untracked-files ssh-port]}]
  (let [files (->> (:out (sh/sh "git" "ls-files"))
    ...
    (->> (concat ["rsync"  "--archive" "--verbose" "--relative" "--include='**.gitignore'"
                 ...
                 (when ssh-port ["-e" (str "ssh -p " ssh-port)])
                 files
                 [(str "app@" server ":")])
         (apply shell))))
...
(defn prod-repl
  ...
  []
  (let [{:keys [biff.tasks/server biff.tasks/ssh-port biff.nrepl/port]} @config]
    (println "Connect to nrepl port" port)
    (spit ".nrepl-port" port)
    (apply shell "ssh" (concat
                        ["-NL" (str port ":localhost:" port)]
                        (when ssh-port ["-p" (str ssh-port)])
                        [(str "app@" server)]))))
